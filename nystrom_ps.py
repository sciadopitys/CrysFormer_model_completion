...
class Nystrom_attention_partial_structure(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, num_landmarks = 64, pinv_iterations = 6, residual = True, residual_conv_kernel = 33, dropout = 0.):
        super().__init__()
        ...
        self.att_mask = nn.Parameter(torch.Tensor(self.heads, self.num_landmarks, self.num_landmarks))
        nn.init.kaiming_uniform_(self.att_mask)
    ...
    def forward(self, x, partial_structure):
        ...
        sim2 = sim2 * torch.sigmoid(self.att_mask).expand(b, -1, -1, -1)
